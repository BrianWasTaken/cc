...
export class StringUtilities extends Utility {
   ...
   */
  public convertToOrdinal = (number: number, commas: boolean): string => {
    let suffix: string;
  ...
  public getPercent = (
    ...
    return `${(Math.round((value / base) * 100) || 0).toFixed(
      fractionDigits,
    )}%`;
  };
   ...
   */
  public createPaddedList = (
    numbers: number[],
    align: StringUtilities.PaddedListAlignment,
  ): StringUtilities.PaddedListItem[] => {
